...
#         self.c_out = CBL(c_ * 4, out_channels, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        x = self.c_out(x)
#         x = self.c_out(torch.cat([x, pool1, pool2, pool3], dim=1))
        return x
...
#         anchors_ = torch.tensor(anchors).float().view(self.nl, -1, 2) / torch.tensor(self.stride).repeat(6, 1).T.reshape(3, 3, 2)
        anchors_ = torch.tensor(anchors).float().view(self.nl, -1, 2) / torch.tensor(self.stride).repeat(6, 1).T.reshape(1, 3, 2)
        self.register_buffer('anchors', anchors_)  # shape(nl,na,2)

        self.out_convs = nn.ModuleList()
        self.out_convs += [
            nn.Conv2d(in_channels=ch, out_channels=(5+self.nc) * self.naxs, kernel_size=1)
        ]
...
#         self.backbone += [
#             CBL(in_channels=1, out_channels=first_out, kernel_size=6, stride=2, padding=2),
#             CBL(in_channels=first_out, out_channels=first_out*2, kernel_size=3, stride=2, padding=1),
#             C3(in_channels=first_out*2, out_channels=first_out*2, width_multiple=0.5, depth=2),  # 160x160x128
#             CBL(in_channels=first_out*2, out_channels=first_out*4, kernel_size=3, stride=2, padding=1), # 80x80x256
#             C3(in_channels=first_out*4, out_channels=first_out*4, width_multiple=0.5, depth=4), # this output is used for the neck low resolution
#             CBL(in_channels=first_out*4, out_channels=first_out*8, kernel_size=3, stride=2, padding=1), # 40x40x512
#             C3(in_channels=first_out*8, out_channels=first_out*8, width_multiple=0.5, depth=6), # this output is used for the neck medium resolution
#             CBL(in_channels=first_out*8, out_channels=first_out*16, kernel_size=3, stride=2, padding=1), # 20x20x1024
#             C3(in_channels=first_out*16, out_channels=first_out*16, width_multiple=0.5, depth=2), # this output is used for the neck high resolution
#             SPPF(in_channels=first_out*16, out_channels=first_out*16) # 20x20x1024
#         ]
# 
        self.backbone += [
            CBL(in_channels=3, out_channels=first_out, kernel_size=3, stride=2, padding=2),
            CBL(in_channels=first_out, out_channels=first_out*2, kernel_size=3, stride=2, padding=1),
            ...
            nn.Linear( first_out*16, 256),
            nn.Linear( 256, 4*4*( 5 + nc ) ) # 4x4 grid
        ]

#         self.neck += [
#             CBL(in_channels=first_out*16, out_channels=first_out*8, kernel_size=1, stride=1, padding=0), # 20x20x1024 -> 20x20x512
#             C3(in_channels=first_out*16, out_channels=first_out*8, width_multiple=0.25, depth=2, backbone=False), # 40x40x512 (concatenate with backbone output)
#             CBL(in_channels=first_out*8, out_channels=first_out*4, kernel_size=1, stride=1, padding=0), #  40x40x512 -> 40x40x256
#             C3(in_channels=first_out*8, out_channels=first_out*4, width_multiple=0.25, depth=2, backbone=False), # 80x80x256 (concatenate with backbone output)
#             CBL(in_channels=first_out*4, out_channels=first_out*4, kernel_size=3, stride=2, padding=1), # 80x80x256 -> 40x40x256
#             C3(in_channels=first_out*8, out_channels=first_out*8, width_multiple=0.5, depth=2, backbone=False), # 80x80x256 (concatenate with backbone output)
#             CBL(in_channels=first_out*8, out_channels=first_out*8, kernel_size=3, stride=2, padding=1), # 40x40x512 -> 20x20x512
#             C3(in_channels=first_out*16, out_channels=first_out*16, width_multiple=0.5, depth=2, backbone=False)    # 40x40x512 (concatenate with backbone output)
#         ]
...
#                 x = torch.cat([x, neck_connection.pop(-1)], dim=1)
            ...
            elif (isinstance(layer, C3_NECK) and idx > 0) or (isinstance(layer, C3) and idx > 0):
                x = layer(x)
                outputs.append(x)
...
if __name__ == "__main__":
    ...
    x = torch.rand(batch_size, 1, image_height, image_width)
...
#                     ch=(first_out*4, first_out*8, first_out*16), inference=False)
                    ch=(first_out*4), inference=False)
