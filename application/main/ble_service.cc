...
bool BLE_Service::AreThereSubscribers() { return notify_enable_; }

// MBFU: In its simplest form, an mbuf is a memory block with some space
// reserved for internal information and a pointer which is used to “chain”
// memory blocks together in order to create a “packet”. This is a very
// important aspect of the mbuf:
//              the ability to chain mbufs together to create larger “packets”
//              (chains of mbufs)
bool BLE_Service::SendNotification() {
  ...
  rc = ble_gatts_notify_custom(BLE::nni::conn_handle,
                               BLE::nni::notification_handle, om);
...
esp_err_t BLE_Service::Start() {
  //! Below is the sequence of APIs to be called to init/enable NimBLE host and
  //! ESP controller:
...
esp_err_t BLE_Service::Stop() {
  //! Below is the sequence of APIs to be called to disable/deinit NimBLE host
  //! and ESP controller:
...
/*             BLE Service Support             */
/***********************************************/
int BLE_Service::gatt_svr_chr_access(uint16_t conn_handle, uint16_t attr_handle,
  ...
  switch (ctxt->op) {
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR: {
      ...
      rc = gatt_svr_chr_write(ctxt->om, min_length, max_length,
...
esp_err_t BLE_Service::startNVS() {
...
int BLE_Service::gatt_svr_chr_write(struct os_mbuf *om, uint16_t min_len,
...
esp_err_t BLE_Service::gatt_svr_init(void) {
  int rc;
...
void BLE_Service::bleprph_advertise(void) {
 ...
 */
int BLE_Service::bleprph_gap_event(struct ble_gap_event *event, void *arg) {
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_DISCONNECT:
      ESP_LOGI(BLE::nni::tag, "disconnect; reason=%d ",
...
void BLE_Service::bleprph_on_reset(int reason) {
  ESP_LOGE(BLE::nni::tag, "Resetting state; reason=%d\n", reason);
}

void BLE_Service::bleprph_on_sync(void) {
  int rc;
...
void BLE_Service::bleprph_host_task(void *param) {
...
static void BLE::nni::bleprph_print_conn_desc(struct ble_gap_conn_desc *desc) {
  ESP_LOGI(BLE::nni::tag,
...
void BLE::nni::gatt_svr_register_cb(struct ble_gatt_register_ctxt *ctxt,
                                    void *arg) {
...
void BLE::nni::print_addr(const void *addr) {
  const uint8_t *u8p;
  u8p = (const uint8_t *)addr;
  ESP_LOGI(BLE::nni::tag, "%02x:%02x:%02x:%02x:%02x:%02x", u8p[5], u8p[4],
